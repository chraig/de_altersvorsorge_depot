...
class _CalculationBreakdownState extends State<_CalculationBreakdown> with TickerProviderStateMixin {
  ...
  Widget _payoutPhase(AppStrings s, PersonalScenario p, AVResult av, ETFResult etf, CostSettings costs, double jbGef, double jbUngef, bool compact) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      _h(s.bdLifetimeTotal(p.rentenalter, CalcConstants.payoutEndAge)),
      _pairFormula(s.bdGrossPayoutTotal,
        Fmt.eur(av.monatlicheAuszahlung * months), Fmt.eur(etf.bruttoMonatlich * months), bold: true,
        avFormula: '= ${Fmt.eur(av.monatlicheAuszahlung)}/mo × $months mo (depot keeps compounding)',
        etfFormula: '= ${Fmt.eur(etf.bruttoMonatlich)}/mo × $months mo (depot keeps compounding)'),
      _pairFormula(s.bdTotalTaxPaid,
        ...
        etfFormula: '= ${Fmt.eur(etf.vorabpauschaleGesamt)} VP + ${Fmt.eur(etfSaleTaxLifetime)} sale tax'),
      _pairFormula(s.bdTaxSavingsPhase,
        '0 €', Fmt.eur(etf.vorabpauschaleGesamt),
        avFormula: 'AV-Depot grows tax-free during savings',
        etfFormula: 'cumulative Vorabpauschale (§18 InvStG)'),
      _pairFormula(s.bdTaxPayoutPhase,
