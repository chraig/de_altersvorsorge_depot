...
class StringsEn extends AppStrings {
  ...
  String get tipVorabpauschale => 'Annual tax on unrealized ETF gains (§18 InvStG): 0.7 × Basiszins × depot value × (1 − Teilfreistellung) × Abgeltungssteuersatz. The displayed drag depends on whether you are kirchensteuerpflichtig (~0.30 % vs ~0.31 %). The cumulative VP paid is credited against the sale tax at the end (§19 Abs. 1 InvStG). Does NOT apply inside the AV-Depot.';
  ...
  String get simplificationsDetail =>
    ...
    'Both: Payout phase uses the ordinary-annuity formula on monthly periods (PMT_m = PV × r_m / (1 − (1 + r_m)⁻ⁿᵐ), where r_m = (1+rendite)^(1/12)−1). Depot keeps compounding at the same rendite during retirement; constant monthly payment depletes it exactly at age 85.\n'
    'ETF: Vorabpauschale simplified as 0.3% annual drag (actual depends on Basiszins)\n'
    'ETF: Sale tax is computed on the lifetime gain (n_m × gross monthly − cost basis) and spread evenly across all payout months for a constant net monthly. Real-life partial sales would have a year-by-year-varying tax burden; only the lifetime total matches.\n'
...
class StringsDe extends AppStrings {
  ...
  String get tipVorabpauschale => 'Jährliche Steuer auf unrealisierte ETF-Gewinne (§18 InvStG): 0,7 × Basiszins × Depotwert × (1 − Teilfreistellung) × Abgeltungssteuersatz. Der angezeigte Abzug hängt davon ab, ob Sie kirchensteuerpflichtig sind (~0,30 % bzw. ~0,31 %). Die kumulierte VP wird bei Verkauf gegen die Abgeltungssteuer angerechnet (§19 Abs. 1 InvStG). Gilt NICHT im AV-Depot.';
  ...
  String get simplificationsDetail =>
    ...
    'Beide: Auszahlphase mit Annuitätenformel auf Monatsbasis (PMT_m = PV × r_m / (1 − (1 + r_m)⁻ⁿᵐ), mit r_m = (1+Rendite)^(1/12)−1). Depot wächst mit derselben Rendite weiter; konstante monatliche Auszahlung läuft das Depot exakt bis Alter 85 leer.\n'
    'ETF: Verkaufssteuer wird auf den Lebenszeit-Gewinn (n_m × Brutto-Monatszahlung − Eigenbeiträge) berechnet und gleichmäßig auf alle Auszahlungsmonate verteilt — konstanter Netto-Auszahlbetrag. Reale Teilverkäufe hätten eine jährlich variierende Steuerbelastung; nur die Gesamtsumme ist exakt.\n'
